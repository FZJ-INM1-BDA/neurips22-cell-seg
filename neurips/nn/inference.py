...
class CpnInference(nn.Module):
    ...
    def process_patch(self, x):
        ...
        score_maps = ensure_float32(y['score_maps'])
        score_maps = [F.interpolate(i[None], x.shape[-2:], mode='nearest')[0] for i in score_maps]
        return contours, scores, boxes, uncertainty, score_maps  # List[Tensor[num, samples, 2]], List[Tensor[num]], List[Tensor[num, 4]]
    ...
    def _forward_tiled(self, x, is_np, size, mask=None):
        ...
        score_maps = torch.zeros((self.reps, 1) + size)
        score_maps_counter = torch.zeros((self.reps, 1) + size)
        ...
        for batch, meta in tq:
            ...
            for n, (contours, scores, boxes, uncertainties, score_map) in enumerate(zip(*outputs)):
                ...
                pad = 128
                slices_ = tuple([slice(s.start + pad * a, s.stop - pad * b) for s, (a, b) in
                                 zip(slices, [(top, bottom), (left, right)])])
                score_maps[(rep_idx, slice(None)) + slices_] += score_map.to(score_maps.device)[:, slice(pad * top, -pad if bottom else None), slice(pad * left, -pad if right else None)]
                score_maps_counter[(rep_idx, slice(None)) + slices_] += 1

                # Remove small boxes
                keep = tv.ops.boxes.remove_small_boxes(boxes, self.min_box_size)
        ...
        score_maps /= score_maps_counter
        score_maps = score_maps.mean(0)
        ...
        return contours, scores, boxes, uncertainties, nms_weights, score_maps
    ...
    def forward(self, x, mask=None):
        ...
        if is_np:
            ...
            size = x.shape[-2:]
        with torch.no_grad():
            contours, scores, boxes, uncertainty, weights, score_maps = self._forward_tiled(x, is_np, size, mask=mask)
        return contours, scores, boxes, uncertainty, weights, score_maps
