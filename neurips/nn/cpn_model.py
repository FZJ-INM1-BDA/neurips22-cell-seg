...
from celldetection.models.fpn import ResNet34FPN, ResNet18FPN, ResNet50FPN, ResNet101FPN, ResNet152FPN, ResNeXt50FPN, \
...
__all__ = [
    ...
    # 'CpnConvNeXtBaseUNet', 'CpnConvNeXtLargeUNet', 'CpnConvNeXtTinyUNet', 'ConvNeXtSmallUNet'
]
...
class CPN(nn.Module):
    ...
    def compute_loss(
        ...
        teacher_scores = targets.get('teacher_scores')  # scores per instance
        teacher_uncertainty = targets.get('teacher_uncertainty')  # uncertainty per instance
        teacher_score_maps = targets.get('teacher_score_maps')  # score map for background
        if teacher_score_maps.ndim == 3:
            teacher_score_maps = teacher_score_maps[:, None]
        teacher_score_maps = F.interpolate(teacher_score_maps, raw_scores.shape[-2:])
        ...
        fg_teacher_scores = teacher_scores[b, fg_indices]  # using instance scores, rather than pixel scores
        bg_teacher_scores = teacher_score_maps[bg_n, 0, bg_y, bg_x]  # Tensor[-1, classes]

        if contour_targets is not None:
            c_tar = contour_targets[b, fg_indices]  # Tensor[num_pixels, samples, 2]
            if box_targets is None:
                box_targets = contours2boxes(c_tar, axis=1)

        if self.score_target_dtype is None:
            if isinstance(objectives['score'], nn.CrossEntropyLoss):
                score_targets_dtype = torch.int64
            else:
                score_targets_dtype = fg_scores.dtype
        else:
            score_targets_dtype = self.score_target_dtype

        keep_score_fg = fg_teacher_scores >= self.teacher_score_thresh_fg
        keep_score_bg = bg_teacher_scores <= self.teacher_score_thresh_bg
        teacher_uncertainty = teacher_uncertainty[b, fg_indices]
        uncertainty_sig = None if uncertainty is None else torch.sigmoid(uncertainty)
        if uncertainty_sig is None:
            uncertainty_sig = torch.ones_like(teacher_uncertainty, device=teacher_uncertainty.device)
        else:
            torch.nan_to_num_(uncertainty_sig, 1., 1., 1.)
        assert torch.all(teacher_uncertainty >= 0.) and torch.all(teacher_uncertainty <= 1.)

        if fg_scores.numel() > 0:
            # keep if teacher more certain than student
            keep_uncertainty = (teacher_uncertainty + self.teacher_uncertainty_margin) <= uncertainty_sig  # relative c.
            # keep only uncertain examples from student
            keep_uncertainty &= (
                    uncertainty_sig > self.student_uncertainty_margin)  # remove certain student predictions
            # always keep if teacher is absolutely certain
            keep_uncertainty |= teacher_uncertainty <= 0.
            # only train contours if all borders meet criteria
            keep_contours = keep_uncertainty.all(1)

            info['keep_contour_ratio'] = keep_contours.float().mean()
            info['keep_boundary_ratio'] = keep_uncertainty.float().mean()

        ones = zeros = None
        if fg_scores.numel() > 0:
            if class_targets is None:
                ones = torch.broadcast_tensors(torch.ones((), dtype=score_targets_dtype, device=fg_scores.device),
                                               fg_scores[..., 0])[0]
            else:
                ones = class_targets[b, fg_indices].to(score_targets_dtype)

            if self.score_channels == 1:
                fg_scores = torch.squeeze(fg_scores, 1)

            bx_keep = remove_small_boxes(boxes, 1., return_mask=True)
            bx_keep_ = torch.logical_not(bx_keep)
            bx_keep &= keep_score_fg
            bx_keep_ &= keep_score_fg
            iou = fun.pairwise_box_iou(boxes[bx_keep], box_targets[bx_keep]).detach()

            if self.box_confidence and self.score_channels == 1:
                ones = torch.clone(ones)
                ones[bx_keep] = iou
                ones[bx_keep_] = .1  # TODO

            add_to_loss_dict(losses, 'score', objectives['score'](fg_scores[keep_score_fg], ones[keep_score_fg]),
                             self.weights['score_fg'])
        if bg_scores.numel() > 0:
            zeros = torch.broadcast_tensors(torch.zeros((), dtype=score_targets_dtype, device=bg_scores.device),
                                            bg_scores[..., 0])[0]
            if self.score_channels == 1:
                bg_scores = torch.squeeze(bg_scores, 1)
            add_to_loss_dict(losses, 'score', objectives['score'](bg_scores[keep_score_bg], zeros[keep_score_bg]),
                             self.weights['score_bg'])

        if self.binary_loss and ones is not None and zeros is not None and self.score_channels == 1:
            binary_loss = objectives['binary'](
                torch.cat((bg_scores[keep_score_bg], fg_scores[keep_score_fg]), 0),
                torch.cat((zeros[keep_score_bg], ones[keep_score_fg]), 0)
            )
            add_to_loss_dict(losses, 'binary', binary_loss, self.weights['binary'])
        ...
        if fg_indices.numel() > 0:
            if fourier_targets is not None:
                ...
                add_to_loss_dict(losses, 'fourier',
                                 (objectives['fourier'](fourier[keep_contours],
                                                        f_tar[keep_contours]) * self.order_weights).mean(),
            ...
            if location_targets is not None:
                ...
                add_to_loss_dict(losses, 'location',
                                 objectives['location'](locations[keep_contours], l_tar[keep_contours]),
                                 self.weights['location'])
            if contour_targets is not None:
                contour_loss = objectives['contour'](contours[keep_contours], c_tar[keep_contours])
                add_to_loss_dict(losses, 'contour', contour_loss, self.weights['contour'])

                if self.iou_loss is not None:
                    assert self.iou_loss in ('giou', 'iou', 'log-giou', 'log-iou')
                    iou_loss = fun.iou_loss(boxes[keep_contours], box_targets[keep_contours], min_size=1.,
                                            generalized='giou' in self.iou_loss,
                                            method='log' if 'log' in self.iou_loss else 'linear')
                    torch.nan_to_num_(iou_loss, 0., 0., 0.)
                    add_to_loss_dict(losses, 'iou', iou_loss, self.weights['iou'])

                if self.box_loss:
                    add_to_loss_dict(losses, 'boxes',
                                     objectives['boxes'](boxes[keep_uncertainty], box_targets[keep_uncertainty]),
                                     self.weights['boxes'])

                if uncertainty is not None:
                    unc_loss = fun.box_npll_loss(uncertainty[keep_contours], boxes[keep_contours].detach(),
                                                 box_targets[keep_contours],
                                                 min_size=1., sigmoid=True,
                                                 sigmoid_factor=self.uncertainty_factor)
                    add_to_loss_dict(losses, 'uncertainty', unc_loss, self.weights['uncertainty'])
                ...
                if self.refinement and self.refinement_iterations > 0:
                    ...
                    ref_contour_loss = objectives['refinement'](refined_contours[keep_contours], cc_tar[keep_contours])
                    add_to_loss_dict(losses, 'refinement', ref_contour_loss, self.weights['refinement'])
        loss = reduce_loss_dict(losses, 1)
        return loss, losses, info
    ...
    def forward(
        ...
        if selected_contours.numel() > 0:
            selected_boxes = contours2boxes(selected_contours, axis=1)  # 43.3 µs ±290 ns for Tensor[2203, 32, 2]
        else:
